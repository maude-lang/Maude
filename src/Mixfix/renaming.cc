...
Renaming::printRenaming(ostream& s, const char* sep, const char* sep2) const
{
  ...
  for (auto om : opMapIndex)
      ...
      if (om->second.mappingType == MappingType::ATTR)
	{
      ...
      s << ((om->second.mappingType == MappingType::MSG) ? "msg " : "op ") << Token::name(om->first);
      if (!om->second.types.empty())
	{
	  s << " :";
