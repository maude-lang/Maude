...
PigPug::extractUnifier(Subst& unifier, ConstraintMap& constraintMap)
{
  ...
  */
  ...
  tightenConstraints(unifier, constraintMap);
  return nextVariableName;
}

void
PigPug::tightenConstraints(const Subst& subst, ConstraintMap& constraintMap)
{
  //
  //	We know by all the checks during construction, our substitution
  //	satisfies the constraints given on the original variables.
  //
  //	But it maybe that these constraints induce tighter constraints
  //	on the variables occuring in the rhs then we currently recognize.
  //
  for (int i = 0; i <= lastOriginalVariable; ++i)
    {
      VariableConstraint vc = constraintMap[i];
      const Word& word = subst[i];
      int wordSize = word.size();
      if (vc.isUnbounded() || (wordSize == 1 && word[0] == i))
	{
	  //
	  //	If the lhs variable has no upperbound on what it can take
	  //	or is free (bound to itself), it can't tighten the constraint
	...
	{
	  //
	  //	We have a variable X with an upperbound b and an assignment
	  //	  X |-> Y
	  //	This can only happen if the chain of assignments from X to
	  //	Y were only generated by EQUATE steps and these propagate
	  //	constraints perfectly. So the constraint on Y must be at
	  //	least as tight as the constraint on X so if we are debugging
	  //	we Assert() this.
	  //
#ifndef NO_ASSERT
	  int rhs = word[0];
	  VariableConstraint meet(constraintMap[i]);
	  bool ok = meet.intersect(constraintMap[rhs]);
	  Assert(ok, "constraint clash for x" << i << " bound to x" << rhs);
	  Assert(constraintMap[rhs] == meet, "imperfect constraint propagation for x" <<
		 i << " bound to x" << rhs);
#endif
	}
      else
	{
	  ...
	  //	We have a variable X with an upperbound b and an assignment
	  //	  X |-> Y1 ... Yk
	  //	Thus Yi should not have a bound greater than 1 + b - k.
	  //	Because splitting of upperbounds > 2 during PEEL steps is imperfect
	  //	it is possible that Yi could have a larger bound in which case
	  //	we can tighten it.
	  //
	  int lhsUpperBound = vc.getUpperBound();
	  Assert(lhsUpperBound >= wordSize, "bounds violation: variable " << i <<
		 " has lhsUpperBound = " << lhsUpperBound <<
		 " and wordSize = " << wordSize);
	  int rhsUpperBound = 1 + lhsUpperBound - wordSize;

	  for (int v : word)
	    {
	      int originalBound = constraintMap[v].getUpperBound();
	      if (originalBound > rhsUpperBound)
		{
		  constraintMap[v].setUpperBound(rhsUpperBound);
		  /*
		  cout << "PigPug lowered constraint on rhs variable " << v <<
		    " from " << originalBound << " to " <<  rhsUpperBound << endl;
		  */
		}
	    }
	}
